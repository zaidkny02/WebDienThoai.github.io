...
namespace TestUserSQL
{
    public partial class WebForm4 : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (Request.QueryString["kw"] != null)
                    key_word = Request.QueryString["kw"].ToString();
                if (key_word.Trim() != "")
                {
                    h3.InnerText = "Kết quả tìm kiếm về ' " + key_word + " '";
                    ...
                    string sql;
                    // data for Paging
                    SqlCommand cmd = new SqlCommand("select count(PK_iMasanpham) as soluong from SANPHAM where sTenSanpham Like '%" + key_word + "%'", Cnn);
                    ...
                    else
                    {
                        ...
                        sql = sql + " WHERE PK_iMasanpham not in (select top " + except + " PK_iMasanpham From SANPHAM where sTenSanpham Like  N'%" + key_word + "%' ";
                        sql = sql + " )";
                        sql = sql + " and HINHANHSP.iHienthi = 1";
                        sql = sql + " and SANPHAM.PK_iMasanpham = HINHANHSP.FK_iMasanpham and sTenSanpham Like '%" + key_word + "%'";
        ...
        protected void repeaterPaging_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            ...
            Response.Redirect("Timkiem_SP.aspx?kw=" + key_word + "&page=" + nextPage + "&order=" + a);
        }
                ...
                case "DESC":
                    {
                        ...
                        sql = sql + " and HINHANHSP.iHienthi = 1";
                        sql = sql + " and sTenSanpham Like  N'%" + key_word + "%' ORDER BY iGiaban-iGiaban*isnull(iTilekhuyenmai,0)/100 " + order;
                        SqlDataAdapter da = new SqlDataAdapter(sql, Cnn);
                        DataTable dt = new DataTable();
                        da.Fill(dt);
                        grvData.DataSource = dt;
                ...
                case "SELL_DESC":
                    {
                        ...
                        sql = sql + " and isnull(KHUYENMAI.dNgaybatdau,'1/1/2000') <= GETDATE() and isnull(KHUYENMAI.dNgayketthuc,'12/12/2100') >= GETDATE()";
                        sql = sql + " where SANPHAM.PK_iMasanpham = HINHANHSP.FK_iMasanpham and sTenSanpham Like  N'%" + key_word + "%'  ";
                        sql = sql + " and HINHANHSP.iHienthi = 1";
