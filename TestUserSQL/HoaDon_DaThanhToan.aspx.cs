...
{
    public partial class HoaDon_DaThanhToan : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                KhoiTaoDuLieu();
                khoiphucBtn.Visible = false;
                xacnhantrahangBtn.Visible = false;
                tuchoitrahangBtn.Visible = false;
            }
        }
           ...
           // sql = sql + " and HOADON.iTrangthai = 3";
            switch (chedoxem)
            {
                case "0":
                    ...
                    break;
                case "1":
                    sql = sql + " and HOADON.iTrangthai = 4";
                    break;
                case "2":
                    sql = sql + " and HOADON.iTrangthai = -4";
                    break;
        ...
        protected string laytrangthaisp(int id, string kieu,string idcu = null)
        {
            ...
            else
            {
                ...
                {
                    if(idcu.Equals("4"))
                        sql = "select iTrangthai from HOADON_LOG where iMahoadon = " + id + " and iTrangthai != " + idcu + " and iTrangthai != -4  order by Ngaycapnhat DESC";
                    else
                        sql = "select iTrangthai from HOADON_LOG where iMahoadon = " + id + " and iTrangthai != " + idcu + "  order by Ngaycapnhat DESC";
                }
            }
            SqlConnection cnn = new SqlConnection(constr);
            SqlCommand cmd = new SqlCommand(sql, cnn);
        ...
        protected void xacnhanbtn_Click(object sender, EventArgs e)
        {
            /*if (taikhoanid != 0)
            {
                ...
                if (checkslsp(id))
                {
                    string sql = "";
                    //get trang thai
                    string trangthai = laytrangthaisp(id, "hientai");
                    switch (trangthai)
                    {
                        case "":
                            break;
                        ...
                        case "1":
                            ...
                            break;
                        case "-1":
                            string trangthaicu = laytrangthaisp(id, "cu","-1");
        ...
        protected void saveBtn_Click(object sender, EventArgs e)
        {
            if (grvHoaDon.SelectedDataKey != null && donhangid != 0)
            {
                ...
                string sql = "update HOADON set sSDT = '" + sdt + "',sDiachi = N'" + diachi + "',iPhuphi =" + phuphi + ",sGhichu = N'" + ghichu + "'  where PK_iMahoadon = " + grvHoaDon.SelectedDataKey.Value.ToString();
                SqlConnection cnn = new SqlConnection(constr);
                SqlCommand cmd = new SqlCommand(sql, cnn);
                cmd.CommandType = CommandType.Text;
                cnn.Open();
        ...
        protected void ddlChedoxem_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                case "0":
                    khoiphucBtn.Visible = true;
                    trahangBtn.Visible = false;
                    xacnhantrahangBtn.Visible = false;
                    tuchoitrahangBtn.Visible = false;
                    break;
                case "1":
                ...
                case "2":
                    khoiphucBtn.Visible = false;
                    trahangBtn.Visible = false;
                    xacnhantrahangBtn.Visible = true;
                    tuchoitrahangBtn.Visible = true;
                    break;

            } 
        }

        protected bool kiemtrahoadon(string donhangid)
        {
            string sql = " select FK_iMataikhoan,iTrangthai from HOADON where PK_iMahoadon = " + donhangid;
            SqlConnection cnn = new SqlConnection(constr);
            SqlCommand cmd = new SqlCommand(sql, cnn);
            cmd.CommandType = CommandType.Text;
            cnn.Open();
            ...
            {
                data.Read();
                mataikhoan = data["FK_iMataikhoan"].ToString();
                trangthai = data["iTrangthai"].ToString();
            }
            cnn.Close();
            if (mataikhoan.Equals("9") && trangthai.Equals("3"))
                return true;
            else
                return false;
        }

        protected void trahangBtn_Click(object sender, EventArgs e)
        {
            if (grvHoaDon.SelectedDataKey != null && donhangid != 0)
            {
                if (kiemtrahoadon(donhangid.ToString()))
                {
                    ...
                    lblThongBaoLoi.Text = "Đơn hàng không hợp lệ";
            }
            else
                lblThongBaoLoi.Text = "Chưa chọn đơn hàng";
        }

        protected void khoiphucBtn_Click(object sender, EventArgs e)
        {
            if (grvHoaDon.SelectedDataKey != null && donhangid != 0)
            {
                string trangthai = laytrangthaisp(donhangid, "hientai");
                if (trangthai.Equals("4"))
                {
                    string trangthaicu = laytrangthaisp(donhangid, "cu", "4");
        ...
        protected void xacnhantrahangBtn_Click(object sender, EventArgs e)
        {
            if (grvHoaDon.SelectedDataKey != null && donhangid != 0)
            {
                string trangthai = laytrangthaisp(donhangid, "hientai");
                if (trangthai.Equals("-4"))
                {
                    ...
                    lblThongBaoLoi.Text = "Trạng thái đơn hàng không phù hợp";
            }
            else
                lblThongBaoLoi.Text = "Chưa chọn đơn hàng";
        }

        protected void tuchoitrahangBtn_Click(object sender, EventArgs e)
        {
            if (grvHoaDon.SelectedDataKey != null && donhangid != 0)
            {
                string trangthai = laytrangthaisp(donhangid, "hientai");
